...
class GeradorCurriculo:
    ...
    def aplicar_negrito(self):
        """Aplica marcador [[B]]...[[/B]] no texto selecionado."""
        try:
            ...
            if isinstance(widget, tk.Text):
                try:
                    ...
                    if content:
                        ...
                        widget.insert("insert", f"[[B]]{content}[[/B]]")
                except tk.TclError:
                    pass
        except:
            pass
    ...
    def aplicar_marcacao(self, texto):
        """Converte marcadores [[B]]...[[/B]] em \textbf{...} após o escape."""
        return texto.replace("[[B]]", r"\textbf{").replace("[[/B]]", "}")
        ...
        try:
            ...
            for item in self.lista_exp:
                ...
                if emp:
                    ...
                    if desc_txt:
                        # Primeiro escapa o texto bruto, depois converte marcadores
                        desc_escapado = self.limpar_latex(desc_txt)
                        desc_final = self.aplicar_marcacao(desc_escapado)
                        bloco += r"\begin{itemize}[leftmargin=12pt, label=\textbullet, itemsep=1pt, parsep=1pt]" + "\n"
                        for linha in desc_final.split('\n'):
                            linha = linha.strip()
                            if linha:
                                bloco += rf"  \resumeItem{{{linha}}}" + "\n"
                        bloco += r"\end{itemize}" + "\n"
            ...
            for l in hab_lines:
                if ":" in l:
                    dom, itens = l.split(":", 1)
                    dom_limpo = self.limpar_latex(dom.strip())
                    itens_limpo = self.limpar_latex(itens.strip())
                    itens_final = self.aplicar_marcacao(itens_limpo)
                    hab_itens.append(rf"\item[{dom_limpo}:] {itens_final}")
            hab_tex = "\n".join(hab_itens) if hab_itens else r"\item Nenhuma habilidade registrada."

            # --- Resumo (com marcadores) ---
            resumo_raw = self.txt_resumo.get("1.0", "end-1c").strip()
            resumo_escapado = self.limpar_latex(resumo_raw) if resumo_raw else "Resumo profissional não informado."
            resumo_final = self.aplicar_marcacao(resumo_escapado)

            # --- Idiomas ---
            idiomas_raw = self.txt_idiomas.get().strip()
            idiomas_final = self.limpar_latex(idiomas_raw) if idiomas_raw else "Idiomas não informados."
            idiomas_final = self.aplicar_marcacao(idiomas_final)
